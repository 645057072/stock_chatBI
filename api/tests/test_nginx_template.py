...
def test_nginx_template_has_envsubst_placeholders():
    ...
    assert "proxy_pass http://$chatbi_upstream/;" not in text
    assert "upstream chatbi_api" not in text


def test_api_location_order_set_rewrite_proxy_pass():
    """set → rewrite → proxy_pass，避免变量未初始化且保证去掉 /api 前缀。"""
    p = _repo_root() / "deploy" / "nginx.conf.template"
    text = p.read_text(encoding="utf-8")
    pos = 0
    for _ in range(2):
        s = text.find("set $chatbi_upstream", pos)
        rw = text.find("rewrite ^/api/", pos)
        r = text.find("proxy_pass http://$chatbi_upstream;", pos)
        assert s != -1 and rw != -1 and r != -1, "每个 server 块须含 set、rewrite、proxy_pass"
        assert s < rw < r
        pos = r + 10
