...
def init_agent_service():
    ...
    try:
        bot = Assistant(
            ...
            # code_interpreter 依赖本机 Docker，无 Docker 时初始化会失败；图表由 exc_sql 内生成
...
def ensure_event_loop_for_gradio():
    """在 Gradio 调用 Blocks.queue() 之前绑定主线程事件循环。

    Python 3.12+ 主线程若未 set_event_loop，asyncio.get_event_loop() 会抛 RuntimeError；
    Gradio 5 的 safe_get_lock()/safe_get_stop_event() 捕获后返回 None，导致
    pending_message_lock 与 stop_event 为空，进而出现 queue_join 与流式响应报错。
    """
    try:
...
def app_gui():
    ...
    # 延迟导入，避免未安装 qwen-agent[gui] 时影响终端模式与模块加载
...
if __name__ == '__main__':
    # 默认尝试 Web；缺少 gradio/modelscope_studio 或安装失败时改用终端（避免直接 Traceback 退出）
    try:
        app_gui()
    except ImportError:
        print('未安装 Web 界面依赖（需要 gradio>=5 与 modelscope_studio）。')
        print('安装可执行: pip install "qwen-agent[gui]"')
        print('当前已切换为终端交互模式。\n')
        app_tui()